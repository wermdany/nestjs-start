 ...
 *   而不是 Nest 默认的字符串数组。失败响应的 `success` / `error` 由
 *   `AppExceptionFilter` 补齐（管道只负责把明细结构化）。
 *
...
/**
 * 造一个配好默认开关的 `ValidationPipe`。
 * 用在参数级/控制器级 `@UsePipes()` 上，或作为不使用 `ApiContractModule` 时的备选。
 */
export function createValidationPipe(
  options: ValidationPipeOptions = {},
): ValidationPipe {
  return new ValidationPipe(resolveValidationPipeOptions(options));
}
